...
# Истина, если ответ пользователя означает «да» (y/yes/да).
# Реализовано без ${var,,} — эта конструкция есть только в bash 4+.
is_yes() {
  case "$1" in
    y | Y | yes | YES | Yes | да | Да | ДА | д | Д) return 0 ;;
    *) return 1 ;;
  esac
}

# Запросить подтверждение (по умолчанию — «нет»). Возвращает 0 при согласии.
confirm() {
