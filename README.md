...
- Ubuntu **22.04 LTS** или **24.04 LTS** (для **26.04** — предупреждение и запрос подтверждения)
- Архитектура **x86_64 (amd64)**
- Права **root** (запуск через `sudo`)
- Свободное место на диске: **≥ 3 ГБ** (порог проверки; реально для полной установки нужно ~10–20 ГБ)
- Оперативная память: **≥ 2 ГБ** (жёсткий минимум; < 2 ГБ — установка отменяется, 2–4 ГБ — разрешена с предупреждением, 4+ ГБ — OK)
  - пороги переопределяются: `MIN_RAM_MB` (по умолчанию 2048), `REC_RAM_MB` (по умолчанию 4096)
...
| ---------------- | ----------- | ------------------------------------------------- |
| Ubuntu 22.04 LTS | x86_64      | Полностью поддерживается                          |
| Ubuntu 24.04 LTS | x86_64      | Полностью поддерживается                          |
| Ubuntu 26.04     | x86_64      | Предупреждение + подтверждение (не тестировалась) |
| Прочие           | —           | Отклоняется установщиком                          |

Версии, требующие подтверждения, задаются в `lib/checks.sh`
(`UBUNTU_CONFIRM_VERSIONS`), полностью поддерживаемые — в `UBUNTU_SUPPORTED_VERSIONS`.
...
### Инициализация Wine идёт долго / «зависает»
Первый запуск `wineboot` создаёт Wine prefix с нуля — это занимает 1–3 минуты
(на слабом VPS дольше). Вывод `wineboot` транслируется в консоль в реальном
времени, а таймаут установлен в 180 секунд (`timeout 180` в `lib/wine.sh`).

Если шаг всё равно кажется «зависшим», проверьте во втором терминале:
```bash
ps aux | grep -E 'wine|Xvfb' | grep -v grep
tail -n 20 /var/log/acc-manager.log
ls -la /home/acc/.wine          # drive_c появляется по мере создания
```
Установщик автоматически добавляет запись `127.0.1.1 <hostname>` в `/etc/hosts`,
если имя хоста не резолвится — это частая причина зависания Wine на серверах.
...
Освободите место и повторно запустите установщик. Порог проверки — 3 ГБ
(переопределяется через `MIN_DISK_GB`), но для полной установки реально нужно
~10–20 ГБ: контент ACC Dedicated Server с DLC занимает несколько гигабайт.
```bash
